...
def checkPrice():
    ...
    while(True):
        ...
        productName = str(title[0]).replace('<div class="flex-grow-1"><h1>', '')
        #Search the line
        ...
        price = str(mydivs[nr]).replace('<dd class="col-6 col-xl-7">', '').replace('€</dd>', '').replace(',', '.').replace('</dd>','')
...
def printTime():
    print(time.ctime())
    print("Nome carta: Jolteon VMAX (Promo, Alternative Art)")
...
link = "https://www.cardmarket.com/it/Pokemon/Products/Singles/SWSH-Black-Star-Promos/Jolteon-VMAX?sellerCountry=17&language=1,5"
headers = {
